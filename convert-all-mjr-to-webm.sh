...
do
  ...
  if [ -f "audio_"$filename ]; then
    ...
    janus-pp-rec "audio_"$filename /tmp/janus-recordings/audio.opus  >/dev/null 2>&1
    RESULT=$?
    printf "  -> \033[33mMerging\033[0m\n"
    ffmpeg -i /tmp/janus-recordings/audio.opus -i /tmp/janus-recordings/video.webm -y -c:v copy -c:a copy $video.webm  >/dev/null 2>&1
    RESULT=$?
    if [ $RESULT -eq 0 ]; then
